...
namespace XLua
{
    public partial class ObjectTranslatorProxy
    {
        ...
        public class IniterAdderUnityEngineVector2
        {
			...
			{
				var proxy = ObjectTranslatorProxy.Find(translator);
			
				proxy.RegisterPushAndGetAndUpdate<UnityEngine.Vector2>(proxy.PushUnityEngineVector2, proxy.Get, proxy.UpdateUnityEngineVector2);
				proxy.RegisterPushAndGetAndUpdate<UnityEngine.Vector3>(proxy.PushUnityEngineVector3, proxy.Get, proxy.UpdateUnityEngineVector3);
				proxy.RegisterPushAndGetAndUpdate<UnityEngine.Vector4>(proxy.PushUnityEngineVector4, proxy.Get, proxy.UpdateUnityEngineVector4);
				proxy.RegisterPushAndGetAndUpdate<UnityEngine.Color>(proxy.PushUnityEngineColor, proxy.Get, proxy.UpdateUnityEngineColor);
				proxy.RegisterPushAndGetAndUpdate<UnityEngine.Quaternion>(proxy.PushUnityEngineQuaternion, proxy.Get, proxy.UpdateUnityEngineQuaternion);
				proxy.RegisterPushAndGetAndUpdate<UnityEngine.Ray>(proxy.PushUnityEngineRay, proxy.Get, proxy.UpdateUnityEngineRay);
				proxy.RegisterPushAndGetAndUpdate<UnityEngine.Bounds>(proxy.PushUnityEngineBounds, proxy.Get, proxy.UpdateUnityEngineBounds);
				proxy.RegisterPushAndGetAndUpdate<UnityEngine.Ray2D>(proxy.PushUnityEngineRay2D, proxy.Get, proxy.UpdateUnityEngineRay2D);
        ...
        public void PushUnityEngineVector2(RealStatePtr L, UnityEngine.Vector2 val)
        {
            ...
            if (!CopyByValueProxy.Pack(buff, 0, val))
            {
        ...
        public void Get(RealStatePtr L, int index, out UnityEngine.Vector2 val)
        {
            ...
            if (type == LuaTypes.LUA_TUSERDATA )
            {
                ...
                IntPtr buff = LuaAPI.lua_touserdata(L, index);if (!CopyByValueProxy.UnPack(buff, 0, out val))
                {
			...
			{
			    CopyByValueProxy.UnPack(this, L, index, out val);
			}
            else
            {
	...
	public partial class StaticLuaCallbacksProxy
    {
	    internal static bool __tryArrayGet(Type type, RealStatePtr L, ObjectTranslator translator, object obj, int index)
		{
			ObjectTranslatorProxy proxy = ObjectTranslatorProxy.proxy;
		...
		internal static bool __tryArraySet(Type type, RealStatePtr L, ObjectTranslator translator, object obj, int array_idx, int obj_idx)
		{
			...
			{
			    UnityEngine.Vector2[] array = obj as UnityEngine.Vector2[];
				proxy.Get(L, obj_idx, out array[array_idx]);
				return true;
			}
			else if (type == typeof(UnityEngine.Vector3[]))
			{
